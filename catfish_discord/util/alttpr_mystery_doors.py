...
def generate_doors_mystery(weights, tournament=True, spoilers="mystery"):
    while True:
        sub_weight_name = get_random_option(
            {k: v['chance'] for (k, v) in weights.get('subweights', {}).items()})

        if sub_weight_name is None:
            break

        sub_weights = weights.get('subweights', {}).get(sub_weight_name, {}).get('weights', {})
        sub_weights['subweights'] = sub_weights.get('subweights', {})
        weights = {**weights, **sub_weights}

    options = {'door_shuffle': get_random_option(weights.get('door_shuffle', 'vanilla')),
               'keydropshuffle': get_random_option(weights.get('keydropshuffle', False)),
               'shopsanity': get_random_option(weights.get('shopsanity', False))}
